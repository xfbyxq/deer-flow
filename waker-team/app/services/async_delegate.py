...
class AsyncDelegateService:
    ...
    async def on_run_completed(self, task: Task) -> None:
        ...
        3. wake_engine.wake() 投递
        4. 投递失败 → TASK.status=failed, failure_reason=delivery_failed
        5. 更新 ledger status
        """
        async with self._db_session_factory() as db:
            ...
            try:
                ...
                await db.commit()
                logger.info(
                    "Wake delivered for ticket %s to parent task %s",
                    current_task.ticket_id, current_task.parent_task_id,
                )
            except WakeDeliveryError as exc:
                # 投递失败
                current_task.status = "failed"
                current_task.result_summary = f"Wake delivery failed: {exc}"
